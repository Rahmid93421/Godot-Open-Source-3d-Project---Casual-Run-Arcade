...
func _ready():
	if(get_parent().get_parent().get_parent().get_parent().characterUnlocked["female2"]):
...
func _on_Buy_mouse_entered():
	get_parent().get_parent().get_node("Panel4/TextAd").bbcode_text = "[center]ARE YOU SURE THAT YOU WANT TO UNLOCK [u][color=red]'PHANTOM'[/color][/u] SKIN for 6500 points?[/center]"
	get_parent().get_parent().get_node("Panel4/AnimationPlayer").play("PanelOut");
	get_parent().get_parent().get_parent().currentCharacter = "female2";

func _on_select_mouse_entered():
	get_node("Panel3/RichTextLabel").bbcode_text = "[center]SELECTED[/center]";
	get_parent().get_parent().get_parent().get_parent().infoDict["skin"] = "female2";
